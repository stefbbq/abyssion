// noise.glsl
// Noise generation functions for procedural effects
// Includes Perlin noise, simplex noise, and fractal noise variants
...
vec2 hash2(vec2 p) {
  return fract(sin(vec2(dot(p, vec2(12.9898, 78.233)), dot(p, vec2(39.346, 11.135)))) * 43758.5453123);
}
...
float turbulence2D(vec2 p, int octaves, float lacunarity, float gain) {
  return abs(fbm2D(p, octaves, lacunarity, gain));
}

float turbulence3D(vec3 p, int octaves, float lacunarity, float gain) {
  return abs(fbm3D(p, octaves, lacunarity, gain));
}
